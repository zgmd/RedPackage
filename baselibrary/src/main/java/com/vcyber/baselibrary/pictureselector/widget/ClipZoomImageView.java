...

{
    ...
    public ClipZoomImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mBitmapFilePath = FileUtils.createRootPath(context);
        mBitmapFileName = String.valueOf(System.currentTimeMillis()) + ".jpg";
     ...
     * @return
     */
    public String onClip(int clipMode) {
        Bitmap clipBitmap = null;
//        Rect rect = new Rect(mHorizontalPadding, mVerticalPadding, getWidth() - 2 * mHorizontalPadding, getWidth() - 2 * mHorizontalPadding);
        if(clipMode==PicSelector.MODE_CLIP_SQUARE){
            Bitmap bitmap = Bitmap.createBitmap(getWidth(), getHeight(),
                    Bitmap.Config.ARGB_8888);
            Canvas canvas = new Canvas(bitmap);
            draw(canvas);
            clipBitmap  = Bitmap.createBitmap(bitmap, mHorizontalPadding,
                    mVerticalPadding, getWidth() - 2 * mHorizontalPadding,
                    getWidth() - 2 * mHorizontalPadding);
        }

        if (clipMode == PicSelector.MODE_CLIP_CIRCLE) {
            mRadius = (getWidth()-mHorizontalPadding*2)/2;
            ...
            //bitmap, mHorizontalPadding,mVerticalPadding, getWidth() - 2 * mHorizontalPadding,getWidth() - 2 * mHorizontalPadding
            clipBitmap = Bitmap.createBitmap(2 * mRadius, 2 * mRadius, Config.ARGB_8888);
            Canvas canvas = new Canvas(clipBitmap);
            RectF f = new RectF(mHorizontalPadding, mVerticalPadding, getWidth() - 2 * mHorizontalPadding, getWidth() - 2 * mHorizontalPadding);
...
//            paint.setShader(new BitmapShader(clipBitmap, BitmapShader.TileMode.CLAMP, BitmapShader.TileMode.CLAMP));
            canvas.drawBitmap(bitmap, null, dst, paint);
//            canvas.drawCircle(mRadius, mRadius, mRadius, paint);
        }
        Logger.e("bitmap-->" + clipBitmap.getByteCount());
        if (FileUtils.saveBitmap(clipBitmap, mBitmapFilePath, mBitmapFileName)) {
            return mBitmapFilePath + File.separator + mBitmapFileName;
        }
        return null;
    }
    ...
    private void checkBorder() {
        ...
        Log.e(TAG, "rect.height() =  " + rect.height() + " ,  height - 2 * mVerticalPadding =" + (height - 2 * mVerticalPadding));
