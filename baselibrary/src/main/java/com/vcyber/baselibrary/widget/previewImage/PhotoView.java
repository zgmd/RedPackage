package com.vcyber.baselibrary.widget.previewImage;
 ...
 * Describe :参考 https://github.com/bm-x/PhotoView
 */
...
public class PhotoView extends ImageView {
    ...
    private void initBase() {
        ...
        Logger.e("photoview-->控件的宽-->"+w+"控件的高--->"+h);
        Logger.e("photoview-->图片的宽-->"+imgw+"图片的高--->"+imgh);
        ...
        //图片宽小于空间的一半，默认放大图片至控件的3/4宽
        if (imgw < w / 2) {
            sx = (float)w*3/4/imgw;
            //此处无意义，只为去最小值，保证宽，高同时缩放
            sy = w*3/4/imgw*2;
        }
    ...
    public void animaFrom(Info info) {
        if (isInit) {
            ...
            if (info.mWidgetRect.width() < info.mImgRect.width() || info.mWidgetRect.height() < info.mImgRect.height()) {
                ...
                ClipCalculate c = info.mScaleType == ScaleType.FIT_START ? new PhotoView.START() : info.mScaleType == ScaleType.FIT_END ? new PhotoView.END() : new PhotoView.OTHER();
    ...
    public void animaTo(Info info, Runnable completeCallBack) {
        if (isInit) {
            ...
            if (info.mWidgetRect.width() < info.mRect.width() || info.mWidgetRect.height() < info.mRect.height()) {
                ...
                final ClipCalculate c = info.mScaleType == ScaleType.FIT_START ? new PhotoView.START() : info.mScaleType == ScaleType.FIT_END ? new PhotoView.END() : new PhotoView.OTHER();
